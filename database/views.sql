...
CREATE OR REPLACE VIEW v_releases AS
    SELECT b.build_name, b.branch_name, b.revision, b.comment, be.timestamp AS build_released, be.comment AS event_comment FROM v_builds b, build_events be, events e WHERE b.id = be.build_id AND e.id = be.event_id AND e.event_name = 'released';
