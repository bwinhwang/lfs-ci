...
CREATE OR REPLACE VIEW v_build_events AS
    SELECT be.id, b.id AS build_id, b.build_name, be.timestamp, e.event_name, be.comment FROM builds b, build_events be, events e where b.id = be.build_id and e.id = be.event_id;
